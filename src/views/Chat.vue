        ...
        <el-scrollbar class="chatContent" ref="scrollBox">
          <ul ref="chatBox" id="scrollBox">
            <li v-for="(item, index) in info" :key="index" :class="[index % 2 === 0 ? 'right' : 'left']">
              ...
              <div v-if="index % 2 !== 0" class="msg-evaluate"  ref="svgRef" :id = item.status >
                <svg width="15" height="20" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" class="icon-mdb"
                ...
                <svg width="15" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                  class="icon-mdb"  style="cursor: pointer; margin-left: 7px;"
                ...
                <svg width="15" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"
                  style="cursor: pointer; margin-left: 7px;" @click=" repeated(index - 1)">
                  <path fill-rule="evenodd" clip-rule="evenodd"
...
const handleThumbUp = (index) => {
  ...
  svgRef.value[(index-1)/2]?.setAttribute('style','display:none')
  const ceshi = svgRef.value[(index-1)/2]
  console.log(ceshi)

  console.log("@@@@@@!!~~~~")
  // svgRef.value[0]?.setAttribute('class','icon-md');
...
const repeated = (index) => {
  ...
  axios({
    ...
    .then((response) => {
      ...
      console.log(itemId)
      // 更新日志的回调，更新的内容
      updateContent(user_id, updatecontent, itemId)
        .then(res => {
          console.log(res)
